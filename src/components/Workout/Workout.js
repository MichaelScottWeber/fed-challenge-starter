...
const Workout = ({ data, selected, handleSelectedWorkout }) => {
    ...
    return (
        <div className={selected ? 'Workout selected' : 'Workout'} onClick={() => handleSelectedWorkout(data.name)}>
            <div className="img-container">
                <div>
                    {/* {renderPlaylistOverlay()} */}
                    <img className="thumb-img" src={data.thumbImg} alt={data.name} />
                </div>
            </div>
            <div className="info-container">
                <div className="title-container">
                    <h2>{data.name}</h2>
                    <img className="trainer-img" src={data.trainerImg} alt={`Trainer for the ${data.name} workout`} />
                </div>
